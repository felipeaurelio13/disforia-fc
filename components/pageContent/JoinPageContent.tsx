        ...
        <div className="grid gap-3 md:grid-cols-3">
          {t.cards.map((card) => (
            <article key={card.title} className="rounded-xl border card-surface p-5">
              ...
              <a href={card.href} target="_blank" rel="noreferrer" className="mt-3 inline-flex min-h-12 items-center rounded-full border border-brand-secondary/16 bg-brand-bg px-4 py-2 text-sm font-semibold text-brand-text/90">
                {card.cta}
              </a>
            </article>
          ))}
